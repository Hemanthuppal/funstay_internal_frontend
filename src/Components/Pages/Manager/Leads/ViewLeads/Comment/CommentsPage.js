...
const navigate = useNavigate();
  ...
  // Fetch lead details (including assignedSalesId) from /leads/:leadid endpoint
  useEffect(() => {
    const fetchLeadDetails = async () => {
      try {
        const leadResponse = await axios.get(`${baseURL}/api/leads/${leadid}`);
        // Assuming the lead details contain an 'assignedSalesId' field
  ...
  const handleAddComment = async () => {
    ...
    const comment = {
      ...
      // managerId: userId, // Uncomment if needed
